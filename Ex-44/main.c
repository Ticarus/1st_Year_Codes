//SELF NOTE: This program does not work maybe you should design your own program not just look and type the same thing in the books you have read


/* This program plays draw poker. Users can play as often as they want, betting between 1 and 5. They are dealt 5 cards and then get to choose which cards to
keep, and which cards to replace. Then new hand is then reviewed and the user's payout is set based on the value of the hand. The user's new bankroll is
...
int main()
{
    ...
    int cardSuit[5];  // Will be one of 4 values (for Clubs, Spades, Hearts, Diamonds)
    int finalRank[5];
    int finalSuit[5];
    ...
    /* This function is called outside the do...while loop because the greeting only needs to be displayed once, while everything else in main will run
...
// Print a quick greeting as well as tell the user the value of different winning hands
...
void printGreeting()
{
    ...
    printf("You are dealt 5 cards, and you then choose which cards too keep or discard.\n");
...
int getBet()
{
    ...
    do  // Will keep running until user enters 0-5
    {
        printf("How much you want to bet? (Enter a number between 1 to 5 or 0 to quit the game):");
        scanf("%d",&bet);
